...
export default function TabNavbarWeb({
    ...
    return (
        <View style={styles.container}>
            ...
            <TouchableOpacity style={styles.boutonRetourcontainer} onPress={() => router.push(`/${pageBack}`)}>
                <Ionicons name="chevron-back" size={27} color="#000000" />
            </TouchableOpacity>

            <View style={styles.ongletsContainer}>
                {onglets.map((onglet) => {
                    const isActive = pathname === `/${onglet.page}`;

                    return (
                        <TouchableOpacity
                            key={onglet.id}
                            style={styles.ongletContainer}
                            onPress={() => !isActive && router.push(`/${onglet.page}`)}
                        >
